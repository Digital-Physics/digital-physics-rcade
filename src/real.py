...
class Real(pygame.sprite.Sprite):
    def __init__(self, x, y, scale, speed):
        ...
        for animation in animation_types:
            ...
            while not end_flag:
                try:
                    img = pygame.image.load(gv.resource_path(f"img/real/{animation}/{i}.png")).convert_alpha()
    ...
    def move(self):
        ...
        # print('x vel', self.x_velocity)

        # print("distance", abs(self.rect.x - gv.khatchig.rect.x), self.in_air)
        if abs(self.rect.x - gv.khatchig.rect.x) < 100 and not self.in_air:
            if self.action == 0:
                self.update_action(1)  # idle/jump
            # else:
        ...
        if self.rect.colliderect(gv.khatchig.rect) and gv.khatchig.vulnerable:
            ...
            # movie.play(3)  # self-contained loop
